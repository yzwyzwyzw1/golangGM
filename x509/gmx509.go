...
import (
	...
	"errors"
	"fmt"
	"github.com/chinaso/golangGM/sm2"
	"github.com/chinaso/golangGM/sm3"
	"github.com/chinaso/golangGM/x509/pkix"

	"github.com/chinaso/golangGM/cryptobyte"
	cryptobyte_asn1 "github.com/chinaso/golangGM/cryptobyte/asn1"
	"io"
	"math/big"
	"net"
	"net/url"
...
func marshalPublicKey(pub *sm2.PublicKey) (publicKeyBytes []byte, publicKeyAlgorithm pkix.AlgorithmIdentifier, err error) {
	...
	publicKeyAlgorithm.Algorithm = oidPublicKeySM2 //借用ECDSA的oidPublicKey
	...
	fmt.Println("publicKeyAlgorithm",publicKeyAlgorithm)
	//switch pub := pub.(type) {
	//case *sm2.PublicKey:
	//	publicKeyBytes = pub.GetUnCompressBytes()
	//	publicKeyAlgorithm.Algorithm = oidPublicKeySM2 //借用ECDSA的oidPublicKey
...
//func MarshalPKIXPublicKey(pub *sm2.PublicKey) ([]byte, error) {
...
const (
	...
	UnknownGMHASHAlgorithm Hash = iota
	SM3
	maxHash
)
...
func (c *Certificate) CheckSignatureFrom(parent *Certificate) error {
	...
	return parent.CheckSignatureSM2(c.SignatureAlgorithm, c.RawTBSCertificate, c.Signature)
}

// CheckSignature verifies that signature is a valid signature over signed from
// c's public key.
func (c *Certificate) CheckSignatureSM2(algo SignatureAlgorithm, signed, signature []byte) error {
	return checkSignaturesm2(algo, signed, signature, c.PublicKey)
}
...
// CheckSignature verifies that signature is a valid signature over signed from
// a crypto.PublicKey.
//func checkSignatureSM2(algo SignatureAlgorithm, signed, signature []byte, publicKey sm2.PublicKey) (err error) {
//	var hashType Hash
...
func checkSignaturesm2(algo SignatureAlgorithm, signed, signature []byte, publicKey *sm2.PublicKey) (err error) {
	...
	if pubKeyAlgo != SM2 { // pubKeyAlgo == SM2
...
func (c *Certificate) CheckCRLSignature(crl *pkix.CertificateList) error {
	...
	return c.CheckSignatureSM2(algo, crl.TBSCertList.Raw, crl.SignatureValue.RightAlign())
}
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (*sm2.PublicKey, error) {
	asn1Data := keyData.PublicKey.RightAlign()

	//fmt.Println("sss",algo)
	switch algo {

	case SM2: //我的SM2使用了ECDSA的iod身份表示
...
func signingParamsForPublicKey(pub interface{}, requestedSigAlgo SignatureAlgorithm) (hashFunc Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	default:
		err = errors.New("gmx509: only SM2 keys supported")
	}
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, priv *sm2.PrivateKey) (cert []byte, err error) {
	...
	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(&priv.PublicKey, template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
	fmt.Println("yyy")
	...
	var signature []byte
	signature, err =priv.Sign(rand,digest,nil)
	if err != nil {
		return
	}
...
func (c *Certificate) CreateCRL(rand io.Reader,priv *sm2.PrivateKey, revokedCerts []pkix.RevokedCertificate, now, expiry time.Time) (crlBytes []byte, err error) {



	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(priv.PublicKey, 0)
	if err != nil {
		return nil, err
	}
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv *sm2.PrivateKey) (csr []byte, err error) {
	...
	fmt.Println("template.SignatureAlgorithm",template.SignatureAlgorithm)
	hashFunc, sigAlgo, err = signingParamsForPublicKey(&priv.PublicKey, template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}

	fmt.Println("hashFunc",hashFunc)
	fmt.Println("sigAlgo",sigAlgo)
...
func getSignatureAlgorithmFromAI(ai pkix.AlgorithmIdentifier) SignatureAlgorithm {
	...
	for _, details := range signatureAlgorithmDetails {
		if ai.Algorithm.Equal(details.oid) {
			//fmt.Println("yyyy",details.pubKeyAlgo)
			return details.algo
		}
	}
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	...
	fmt.Println("in.SignatureAlgorithm",in.SignatureAlgorithm)
	//解析证书请求
...
func (c *CertificateRequest) CheckSignature() error {
   ...
   //case sm2.PublicKey:
	//  return checkSignatureSM2(c.SignatureAlgorithm, c.RawTBSCertificateRequest, c.Signature, c.PublicKey)
   //}
