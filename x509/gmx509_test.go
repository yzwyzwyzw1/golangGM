...
//`
//
var testPrivateKey *sm2.PrivateKey
...
//	//}
//
//	if testPrivateKey ,err := sm2.GenerateKey(rand)
//}


func TestOutputSM2PrivateKey(t *testing.T){

	testPrivateKey ,err := sm2.GenerateKey(rand.Reader)
	if err != nil {
			panic("Failed to create sm2 private key: " + err.Error())
	}
	var pwd []byte
	bool,err:=sm2.WritePrivateKeytoPem("sm2pubkey",testPrivateKey,pwd)//有密钥模式下，需要使用AES家解密，所以这不是我想想要的
	if err != nil {
		t.Error("failed")
	}
	if bool ==true {
...
func TestCreateSelfSignedCertificate(t *testing.T) {
	...
	for _, test := range tests {
       ...
       //3.创建证书二进制数据
		derBytes, err := CreateCertificate(random, &template, &template, test.priv)
		if err != nil {
...
func TestCreateCertificateSM2withSM3(t *testing.T) {
	...
	ok, _ := CreateCertificateToPem("cert.pem", &template, &template, sm2Privkey)
	if ok != true {
...
func CreateCertificateToPem(FileName string, template, parent *Certificate,  privKey *sm2.PrivateKey) (bool, error) {
	der, err := CreateCertificate(rand.Reader, template, parent, privKey)
	if err != nil {
		return false, err
	}
	block := &pem.Block{
...
func (c *Certificate) CheckCertSignature() (err error){
	h := sm3.New()
	...
	if rest, err := asn1.Unmarshal(c.Signature, sm2Sig); err != nil {
		return err
	} else if len(rest) != 0 {
	...
	if !c.PublicKey.Verify(digest,c.Signature) {
...
func TestCreateCertificateRequestSM2withSM3(t *testing.T) {
	random := rand.Reader
	...
	fmt.Printf("sm2Content.sigAlgo:%d",sm2Content.sigAlgo)
	//2.创建证书模板
	...
	fmt.Println("PublicKeyAlgorithm",out.SignatureAlgorithm)
...
//	derBytes, err := CreateCertificate(rand.Reader, template, template, testPrivateKey)
//	if err != nil {
